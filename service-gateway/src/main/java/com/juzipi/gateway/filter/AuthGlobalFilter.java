package com.juzipi.gateway.filter;
...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(antPathMatcher.match("/**/inner/**", path)) {
            ...
            return out(response, ResultCode.USER_NO_PERMISSION);
        }
    ...
    private Mono<Void> out(ServerHttpResponse response, ResultCode resultCode) {
        Result fail = ResultTools.failCustomize(resultCode);
        byte[] bits = JSONObject.toJSONString(fail).getBytes(StandardCharsets.UTF_8);
